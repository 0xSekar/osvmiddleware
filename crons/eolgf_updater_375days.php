...
$lot = count($list);
if($lot>0){
    $newlist = backOff('B', $list, $newlist, 7, 45, TRUE, 0, 375);
    $newlist = backOff('B', $list, $newlist, 30, 180, TRUE, 45, 375);
    $newlist = backOff('B', $list, $newlist, 300, 180, FALSE, 0, 375);
...
function listOfTickers(){
    ...
    foreach ($ids as $key => $value) {
        try {
            $res = $db->prepare("SELECT ticker FROM tickers WHERE id = '".$value['ticker_id']."'");            
            $res->execute();
        } catch(PDOException $ex) {
            echo " Database Error"; //user message
            die("Line: ".__LINE__." - ".$ex->getMessage());
