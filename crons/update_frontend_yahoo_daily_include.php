...
function update_yahoo_daily($pticker = NULL, $force_qhist = false) {
    ...
    while ($row = $res->fetch(PDO::FETCH_ASSOC)) {	
        ...
        if($yquery) {
            ...
            if(isset($response->query) && isset($response->query->results) && (is_array($response->query->results->quote) || ($response->query->results->quote instanceof stdClass))) {
    ...
    try {
        if ($force_qhist) {
            $res = $db->query("SELECT * FROM tickers t INNER JOIN tickers_control tc ON t.id = tc.ticker_id WHERE is_old = FALSE $addq order by ticker");
        } else {
